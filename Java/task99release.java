...
public class task99release {
    public static void main(String[] args) {
        ...
        while (!queue.isEmpty()){
            ...
            System.out.printf("Обработка шага %s [step = %d]\n", cur, step);

            if(cur.equals(vend) || vend == null) break;

            if(cur.z < (h - 1) && maze[cur.z + 1][cur.y][cur.x] == WAY) {
                queue.add(cur.zplus());
                maze[cur.z + 1][cur.y][cur.x] = step + 1;
                if(cur.zplus().equals(vend)) { step++;  break; }
                System.out.printf("--> Можно прыгнуть вниз %s [%d]\n", cur.zplus(), maze[cur.z + 1][cur.y][cur.x]);
            }
            ...
            if(cur.y < (m - 1) && maze[cur.z][cur.y + 1][cur.x] == WAY) {
                ...
                System.out.printf("--> Можно шагнуть вниз %s [%d]\n", cur.yplus(), maze[cur.z][cur.y + 1][cur.x]);
            }
            ...
            if(cur.y > 0 && maze[cur.z][cur.y - 1][cur.x] == WAY) {
                ...
                System.out.printf("--> Можно шагнуть вверх %s [%d]\n", cur.yminus(), maze[cur.z][cur.y - 1][cur.x]);
            }
            ...
            if(cur.x < (n - 1) && maze[cur.z][cur.y][cur.x + 1] == WAY) {
                ...
                System.out.printf("--> Можно шагнуть вправо %s [%d]\n", cur.xplus(), maze[cur.z][cur.y][cur.x + 1]);
            }
            ...
            if(cur.x > 0 && maze[cur.z][cur.y][cur.x - 1] == WAY) {
                ...
                System.out.printf("--> Можно шагнуть влево %s [%d]\n", cur.xminus(), maze[cur.z][cur.y][cur.x - 1]);
            }
        }
        System.out.printf("vent %s\n", vend);
        System.out.printf("Answer %d", 5*step);
    }
}
